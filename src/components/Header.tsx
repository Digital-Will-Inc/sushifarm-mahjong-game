...
const Header = () =>
{
  ...
  {
    const logoSection = responsiveSizes.logoSize + (containerDimensions.width > 300 ? 120 : 0);
    const userSection = responsiveSizes.avatarSize + (containerDimensions.width > 250 ? 100 : 0);
    const totalButtonWidth = responsiveSizes.buttonSize * 4;
  ...
  {
    // Close dropdown first, then show modal with a small delay
    setDropdownOpen(false);
    setTimeout(() =>
    {
  ...
  return (
    <>
      {/* Header container with proper z-index */}
      <div
        ref={headerRef}
        className="relative w-full bg-gradient-to-br from-[#8B4513] via-[#704337] to-[#5D2E1F] shadow-xl h-full z-50"
      >
        ...
        >
          ...
          <div className="relative z-20" ref={dropdownRef}>
            <button
              ...
              className="group flex items-center bg-white/10 backdrop-blur-sm border border-white/20 rounded-full text-white hover:bg-white/20 hover:scale-105 transition-all duration-300 shadow-lg relative z-20"
              style={{
            ...
            {/* Dropdown rendered within header context - lower z-index */}
            {isDropdownOpen && (
              <div className="absolute right-0 top-full mt-3 w-48 bg-white/95 backdrop-blur-md rounded-xl shadow-2xl border border-white/20 overflow-hidden animate-in slide-in-from-top-2 duration-200 z-30">
                <div className="py-1">
